...
export default function HelperLocationDetail() {
  ...
  const { selectedDistricts, addDistrict, removeDistrict, getTotalSelectedCount } =
      ...
      <div className='w-full text-left'>
        <span>
          ...
          {Object.entries(selectedDistricts)
            ...
            .map(([cityName, districts]) => districts.map((district) => `${cityName}-${district}`))
            .flat()
            .join(', ') || '없음'}
        </span>
        <span>
          ({getTotalSelectedCount()}/{MAX_SELECTIONS})
        </span>
      </div>

      <Button className='w-full' onClick={() => navigate('/helper/location')}>
        선택 완료
      </Button>
    </main>
  );
