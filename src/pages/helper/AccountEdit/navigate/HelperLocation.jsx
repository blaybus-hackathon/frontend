...
export default function LocationSelector() {
  // TODO : 폰트 line-height: 150%; /* 25.5px */
  // TODO : 초기화 버튼
  // TODO : 데이터 추가
  ...
  return (
    <main className="max-w-md mx-auto flex flex-col items-center gap-4 p-4">
      ...
      <div className="flex flex-col items-start gap-2.5 self-stretch">
          ...
          <span className="helper-subtitle text-left " >돌봄워크가 나에게 맞는 일자리를 소개시켜 드릴게요. (최대 5개 지역 선택 가능)</span>
        </div>
      ...
      {selectedCity && selectedDistrict ? (
        <div className="w-full grid grid-cols-3 gap-2">
          {LOCATION_DATA[selectedCity][selectedDistrict].map((subDistrict) => {
            const isSelected =
              selectedDistricts[selectedCity]?.[selectedDistrict]?.includes(
                subDistrict
              );
            return (
              <Button
