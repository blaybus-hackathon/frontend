...
export default function Footer({ isManager = false }) {
  ...
  const baseItemClass =
    'hover:bg-black/40 w-full h-full flex flex-col items-center justify-center rounded-[1.25rem] cursor-pointer gap-1';
  const iconClass = 'w-[1.3rem] h-[1.3rem] object-contain';
  const textClass = 'text-white text-[0.7rem] md:text-[0.82rem] font-semibold';

  return (
    <footer
      className={`w-full bg-[var(--main)] h-[4.44rem] rounded-t-[1.25rem] py-3 grid items-center gap-1 ${
        isManager ? 'grid-cols-6' : 'grid-cols-2'
      }`}
    >
      <Link to={homeLink} className={baseItemClass}>
      ...
      {isManager && (
        <>
          ...
          <Link to='/center/elder-register' className={baseItemClass}>
            <img src={RegisterIcon} alt='register' className={iconClass} />
            <span className={textClass}>어르신 등록</span>
          </Link>

          <Link to='/center/matching-info' className={baseItemClass}>
